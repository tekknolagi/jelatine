 ...
 * \returns A pointer to a newly created linked_list_t object */

linked_list_t *ll_create( void )
{
    return gc_malloc(sizeof(linked_list_t));
} // ll_create()

/** Disposes of a linked list and all of its elements
 * \param ll A pointer to a linked_list_t object */

void ll_dispose(linked_list_t *ll)
{
    ll_empty(ll);
    gc_free(ll);
} // ll_dispose()

/** Empties a linked list freeing all of its elements
 * \param ll A pointer to a linked_list_t object */

void ll_empty(linked_list_t *ll)
{
    ll_elem_t *elem = ll->head;
    ll_elem_t *next;
...
void ll_prepend(linked_list_t *ll, void *data)
{
    ll_elem_t *elem = gc_malloc(sizeof(ll_elem_t));
...
/** Adds an element only if it is not already in the list
 * \param ll A pointer to the linked list
 * \param data The new element's data */

void ll_add_unique(linked_list_t *ll, void *data)
{
    for (ll_elem_t *elem = ll->head; elem != NULL; elem = elem->next) {
        if (elem->data == data) {
            return;
        }
    }
...
/** Gets the first element in the list and removes it
 * \param ll A pointer to the linked list
 * \returns The data held by the first element */
...
void *ll_peek_first(const linked_list_t *ll)
{
    if (ll->head == NULL) {
        return NULL;
    } else {
...
size_t ll_size(const linked_list_t *ll)
{
    return ll->count;
} // ll_size()
