...
/** Implements type-specific layout for instances of the java.lang.Thread class
 * \param fm A pointer to the class' field manager */

void fm_layout_java_lang_Thread(field_manager_t *fm)
{
    field_t *fields = fm->instance_fields;

    assert(strcmp(fields[0].name, "runnable") == 0);
...
void fm_layout_java_lang_String(field_manager_t *fm)
{
    field_t *fields = fm->instance_fields;

    assert(strcmp(fields[0].name, "next") == 0);
    assert(strcmp(fields[1].name, "value") == 0);
    assert(strcmp(fields[2].name, "count") == 0);
    assert(strcmp(fields[3].name, "cachedHashCode") == 0);
    assert(strcmp(fields[4].name, "offset") == 0);

    fields[0].offset = JAVA_LANG_STRING_NEXT_OFFSET;
    fields[1].offset = JAVA_LANG_STRING_VALUE_OFFSET;
    fields[2].offset = JAVA_LANG_STRING_COUNT_OFFSET;
    fields[3].offset = JAVA_LANG_STRING_CACHEDHASHCODE_OFFSET;
    fields[4].offset = JAVA_LANG_STRING_OFFSET_OFFSET;
} // fm_layout_java_lang_String()

/** Implements type-specific layout for instances of the java.lang.ref.Reference
 * class
 * \param fm A pointer to the class' field manager */

void fm_layout_java_lang_ref_Reference(field_manager_t *fm)
{
    assert(strcmp(fm->instance_fields[0].name, "referent") == 0);

    fm->instance_fields[0].offset = JAVA_LANG_REF_REFERENCE_REFERENT_OFFSET;
} // fm_layout_lang_ref_Reference()

/** Implements type-specific layout for instances of the
 * java.lang.ref.WeakReference class
 * \param fm A pointer to the class' field manager */

void fm_layout_java_lang_ref_WeakReference(field_manager_t *fm)
{
    assert(strcmp(fm->instance_fields[0].name, "next") == 0);

    fm->instance_fields[0].offset = JAVA_LANG_REF_WEAKREFERENCE_NEXT_OFFSET;
} // fm_layout_java_lang_ref_WeakReference()

/** Implements type-specific layout for instances of the java.lang.Throwable
 * class
 * \param fm A pointer to the class' field manager */

void fm_layout_java_lang_Throwable(field_manager_t *fm)
{
    assert(strcmp(fm->instance_fields[0].name, "detailMessage") == 0);

    fm->instance_fields[0].offset = JAVA_LANG_THROWABLE_DETAILMESSAGE_OFFSET;
} // fm_layout_java_lang_Throwable()
...
void fm_layout_jelatine_VMResourceStream(field_manager_t *fm)
{
    field_t *fields = fm->instance_fields;

    assert(strcmp(fields[0].name, "handle") == 0);
    assert(strcmp(fields[1].name, "resource") == 0);
    assert(strcmp(fields[2].name, "available") == 0);

    fields[0].offset = JELATINE_VMRESOURCESTREAM_HANDLE_OFFSET;
    fields[1].offset = JELATINE_VMRESOURCESTREAM_RESOURCE_OFFSET;
    fields[2].offset = JELATINE_VMRESOURCESTREAM_AVAILABLE_OFFSET;
} // fm_layout_jelatine_VMResourceStream()
