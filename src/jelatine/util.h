...
static inline size_t size_ceil_div(size_t x, size_t y)
{
    return (x + y - 1) / y;
} // size_ceil_div()

/** Rounds x towards the smallest y larger than x
 * \param x The dividend
 * \param y The divisor
 * \param x rounded towards the smallest y larger than x */

static inline size_t size_ceil_round(size_t x, size_t y)
{
    return ((x + y - 1) / y) * y;
} // size_ceil_round()
